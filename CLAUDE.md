...
- `nextflow.config` — single source of truth for `db_list` (legacy), `graph_models`, `machine_learning_models`, `machine_learning_features`, `published_features`, `large_features`, `embeddings`, `domainsplit_run`, `min_embedding_coverage`, `ppi_score_cutoff`, `skip`, `out_dir`, profiles.
