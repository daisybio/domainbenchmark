...
def build_atom_graph(domain, distance_threshold=5.0):
    # Build a graph where nodes are atoms
    # Edges exist if atoms are within a certain distance threshold (e.g., 5 Å)
    # Return 2D distance matrix
    atoms = list(domain.get_atoms())
    graph = [[0.0 for _ in atoms] for _ in atoms]
...
def extract_features(conn: sqlite3.Connection, out_file: h5py.File):
    ...
    for domain_id, protein_id, pdb_af_gz, pdb_rf_gz in domain_protein_df.itertuples(index=False):
        ...
        for pdb_gz in pdb_files:
            ...
            contact_matrix = build_atom_graph(structure)

            # Call matrix encoding 
            fingerprints, adjacency_norm, fp_dict = encode_weighted_graph(contact_matrix)
    ...
    print(f"contact_map_atom: wrote {len(domain_protein_df)} entries")
