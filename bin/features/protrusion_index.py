...
def calculate_protrusion_index(domain, R=10.0):
    ...
    np.fill_diagonal(dist_matrix, np.inf)  # exclude self-distance (i != j)

    count_within_R = (dist_matrix <= R).sum(axis=1)

    Vint = count_within_R * 20.1  # Mean atomic volume in proteins
    Vext = (4 / 3) * np.pi * (R ** 3) - Vint  # Volume of sphere minus Vint

    protrusion_index = np.where(Vint > 0, Vext / Vint, 0.0)
    return protrusion_index.astype(np.float32)
...
def extract_features(conn: sqlite3.Connection, out_file: h5py.File, seed: int, struct_file: h5py.File):
    ...
    for ddi_id, instance_id_a, instance_id_b, pfam_id_a, pfam_id_b in ddi_df.itertuples(index=False):
        ...
        protrusion_index = calculate_protrusion_index(structure)
        feature_vector = pool_residue_feature(np.array(protrusion_index, dtype=np.float32))
        embeddings.write_interaction_instance(out_file, pfam_id_a, pfam_id_b,
                                            instance_id_a, instance_id_b,
                                            feature_vector)
        n_written += 1

    print(f"protrusion_index: wrote {n_written} entries")