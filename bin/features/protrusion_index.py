...
def calculate_protrusion_index(domain, R=10.0):
    ...
    num_atoms = len(atoms)
    protrusion_index = np.zeros(num_atoms, dtype=np.float32)
...
def extract_features(conn: sqlite3.Connection, out_file: h5py.File):
    ...
    for domain_id, protein_id, pdb_af_gz, pdb_rf_gz in domain_protein_df.itertuples(index=False):
        ...
        for pdb_gz in pdb_files:
            ...
            protrusion_index = pad_vector(protrusion_index, max_length)
            vector_list.append(protrusion_index)
    ...
    print(f"protrusion_index: wrote {len(domain_protein_df)} entries")
