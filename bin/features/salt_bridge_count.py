...
def salt_bridge_count(structA, structB, threshold=5.5):
...
def extract_features(conn: sqlite3.Connection, out_file: h5py.File):
    ...
    for domain_id_a, domain_id_b, protein_id_a, protein_id_b, source, pdb_gz in domain_structure.itertuples(index=False):
        ...
        salt_bridge_count = salt_bridge_count(structA, structB, threshold=5.5)
        feature_vector = np.array([salt_bridge_count], dtype=np.float32)
    ...
    print(f"salt_bridge_count: wrote {len(domain_structure)} entries")
