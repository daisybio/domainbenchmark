...
def calculate_clash_heavy_atoms(structure, overlap_tolerance=0.4):
    chain_a_atoms = [a for a in structure[0]['A'].get_atoms() if a.element.strip() != 'H']
    chain_b_atoms = [a for a in structure[0]['B'].get_atoms() if a.element.strip() != 'H']

    ns = NeighborSearch(chain_b_atoms)

    clashes = 0
    # search_radius = max(vdw_radii.values()) * 2 - overlap_tolerance  # max possible clash distance
...
def extract_features(conn: sqlite3.Connection, out_file: h5py.File):
    ...
    for domain_id_a, domain_id_b, protein_id_a, protein_id_b, source, pdb_gz in domain_structure.itertuples(index=False):
        ...
        clashes = calculate_clash_heavy_atoms(structure, 0.5)
    ...
    print(f"clash_heavy_atoms: wrote {len(domain_structure)} entries")
