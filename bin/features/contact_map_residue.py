...
def build_residue_contact_graph(domain, distance_threshold=5.0):
...
def extract_features(conn: sqlite3.Connection, out_file: h5py.File):
    ...
    for domain_id, protein_id, pdb_af_gz, pdb_rf_gz in domain_protein_df.itertuples(index=False):
        ...
        for pdb_gz in pdb_files:
            ...
            contact_matrix = build_residue_contact_graph(structure)
    ...
    print(f"contact_map_residue: wrote {len(domain_protein_df)} entries")
