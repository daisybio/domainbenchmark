...
def calculate_sasa_residue_level(domain):
    ...
    sasa_values = np.zeros(len(list(domain.get_residues())), dtype=np.float32)
    for i, residue in enumerate(domain.get_residues()):
        sasa_values[i] = residue.sasa
    return sasa_values
...
def extract_features(conn: sqlite3.Connection, out_file: h5py.File):
    ...
    for domain_id, protein_id, pdb_af_gz, pdb_rf_gz in domain_protein_df.itertuples(index=False):
        ...
        for pdb_gz in pdb_files:
            ...
            sasa_residue = calculate_sasa_residue_level(structure)
            sasa_residue = pad_vector(sasa_residue, max_length)
            vector_list.append(sasa_residue)
    ...
    print(f"sasa_residue: wrote {len(domain_protein_df)} entries")
