...
def main():
    ...
    if args.per_model_metrics:
        ...
        if per_model_metrics_enrichment:
            ...
            block_ids = write_enrichment_plotly_blocks(
                per_model_metrics_enrichment, args.out_dir, db_name=db_name, prefix=ENRICHMENT_PREFIX
            )
            if block_ids:
                logging.info(f"[OK] Wrote {len(block_ids)} MultiQC enrichment blocks (Plotly-in-MultiQC).")
            else:
